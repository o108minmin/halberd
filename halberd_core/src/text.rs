...
mod tests {
    ...
    #[test]
    // 入力値が正常だったとき
    fn normal_generate_subtitle_from_txt() {
        let dir = tempdir().unwrap();
    ...
    #[test]
    // 入力値が異常だったとき(txtファイルが存在しない)
    fn error_generate_subtitle_from_txt_not_found() {
        let dir = tempdir().unwrap();
        let result = generate_subtitle_from_txt(dir.path().join("test.txt"));
        assert!(result.is_err());
        dir.close().unwrap();
    }
    ...
    #[test]
    // 入力値が正常だったとき
    fn normal_generate_subtitle_from_txt_shift_jis() {
        let dir = tempdir().unwrap();
        let input = dir.path().join("test.txt");
        let mut file = File::create(&input).unwrap();
    ...
    fn normal_generate_subtitle_from_txt_shift_jis_multi_byte() {
        let dir = tempdir().unwrap();
        let input = dir.path().join("test.txt");
        let mut file = File::create(&input).unwrap();
        let expected = "テスト";
    ...
    // 入力値が正常だったとき(txtファイルの中身がutf-8)
    // 入力値がShift_JISかつ英数字の時は文字化けしない
    fn normal_generate_subtitle_from_txt_shift_jis_unicode() {
        let dir = tempdir().unwrap();
        let input = dir.path().join("test.txt");
        let mut file = File::create(&input).unwrap();
    ...
    // 入力値が正常だったとき(txtファイルの中身がutf-8)
    // 入力値がutf-8かつマルチバイトの時は文字化けした状態で出力される
    fn normal_generate_subtitle_from_txt_shift_jis_unicode_multi_byte() {
        let dir = tempdir().unwrap();
        let input = dir.path().join("test.txt");
        let mut file = File::create(&input).unwrap();
        let mb = "テスト";
    ...
    #[test]
    // 入力値が異常だったとき(txtファイルが存在しない)
    fn error_generate_subtitle_from_txt_shift_jis_not_found() {
        let dir = tempdir().unwrap();
        let result = generate_subtitle_from_txt_shift_jis(dir.path().join("test.txt"));
        assert!(result.is_err());
        dir.close().unwrap();
    }
}
